...
if (isset($_GET['action']) && $_GET['action'] == 'search') {
    ...
    if ($method == "index_searcheddata") {
        ...
        $fetch_post = $obj->qry_fetchRows("SELECT * FROM tbl_post WHERE title LIKE '%" . $_GET['data'] . "%' AND status = '1' AND publish='1'");
        if ($fetch_post != 0) {
            ?>
            <?php @$screen_resolution = $_COOKIE['screen_res']; ?>
            <div class="row-height">
...
//                                        }
        ...
        $search_data = $_GET['data'];
        $fetch_post = $obj->qry_fetchrows("select * from tbl_post where find_in_set($userid,isliked) AND status = '1' AND publish='1' AND (title LIKE '%" . $search_data . "%' or kicker LIKE '%" . $search_data . "%' or source LIKE '%" . $search_data . "%' or description LIKE '%" . $search_data . "%') order by sort_order asc");
        //echo "<pre>";
...
//                                        }
        ...
        $userid = $user_detail['userid'];
        $search_data = $_GET['data'];
        $fetch_post = $obj->qry_fetchrows("select * from tbl_post where find_in_set($userid,isbookmarked) AND status = '1' AND publish='1' AND (title LIKE '%" . $search_data . "%' or kicker LIKE '%" . $search_data . "%' or source LIKE '%" . $search_data . "%' or description LIKE '%" . $search_data . "%') order by sort_order asc");
        //echo "<pre>";
        //print_r($fetch_post);exit;
        if ($fetch_post != 0) {
            $count = count($fetch_post);
