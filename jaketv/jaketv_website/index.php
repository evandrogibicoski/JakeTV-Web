...
if (isset($_SESSION['sessionid'])) {
    ...
    if (!empty($all_post_id1)) {
        ...
        $fetch_post = $obj->qry_fetchRows("SELECT * FROM tbl_post WHERE postid IN (" . $d . ") AND publish='1' order by sort_order asc LIMIT $PageLimit");
        $total_post_rows = $obj->qry_numrows("SELECT * FROM tbl_post WHERE postid IN (" . $d . ") AND publish='1' order by sort_order asc");
    } else {
        @$fetch_post == array();
        $total_post_rows = 0;
    }
