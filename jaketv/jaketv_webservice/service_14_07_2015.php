...
switch($METHODforCASE) 
{
		...
		if(isset($userid) && $userid=="0")  // New Registration
		{
			...
			if($googleplusid != 0 )//for social check Google
			{
				...
				if($qry044 > 0)
				{
					...
					$param = array('success' =>1,'msg' =>'Profile Update successfully.','userid' =>$qry045['userid'],'fname'=>$fname,'lname'=>$lname,'email'=>$email,'password'=>'','picture'=>$qry045['picture']);
				}
				else
				{
					...
					if($qry4 > 0) // check exist or not, if not then insert new, else pass all data of same Google id user's
					{
						...
						$param = array('success' =>1,'msg' =>'Login Successfully.','userid' =>$qry5['userid'],'fname'=>$qry5['fname'],'lname'=>$qry5['lname'],'email'=>$qry5['email'],'password'=>$qry5["password"],'picture'=>$qry5["picture"]); //Response
					}
					else
					{
						$picture="";
	...
	case 'GetCategory':
		if(isset($userid))
		{
			...
			foreach($qry17 as $qry17_data)
			{
				...
				$qry18 = qry_numRows("SELECT * FROM `tbl_sub_category` WHERE `status`='1' AND `catid`='".$qry17_data["catid"]."'");
				if($qry18 > 0)
				{
					$qry19 = qry_fetchRows("SELECT * FROM `tbl_sub_category` WHERE `status`='1' AND `catid`='".$qry17_data["catid"]."' ORDER BY `cr_date`");
	...
	case 'GetPost':
		if(isset($userid) && $userid!="")
		{
			...
			$qry02 = qry_fetchRows("SELECT `catid` FROM `tbl_category` WHERE `selected`='1'");
			...
			/*Pagination Sneaker*/
			$PageLimit =10;
			$numtotal=qry_numRows("SELECT `tp`.*,`tc`.`catid`,`tc`.`category`,`tsc`.`subcatid`,`tsc`.`subcategory` FROM `tbl_category` AS `tc` 
								    INNER JOIN `tbl_post` AS `tp` ON `tc`.`catid`=`tp`.`catid` 
								    INNER JOIN `tbl_sub_category` AS `tsc` ON `tp`.`subcatid`=`tsc`.`subcatid`
			...
			foreach($qry222 as $qry21_data)
			{
				...
				$alldata["catid"] = $qry21_data["catid"];
				$alldata["category"] = $qry21_data["category"];
				$alldata["subcatid"] = $qry21_data["subcatid"];
	...
	case 'GetPostByCategory':
		if(isset($userid))
		{
			if(isset($catid) && $catid!="0")
			{
				/*Pagination Sneaker*/
				$PageLimit = 10;
				$numtotal=qry_numRows("SELECT * FROM `tbl_post` WHERE `status`='1' AND `catid`='".$catid."'");
				$totalpage=$numtotal/$PageLimit;
				if($numtotal%$PageLimit!=0)
				{
					$totalpage=explode(".",$totalpage);
				...
				if($numtotal > 0)
				{	
					$qry21 = qry_fetchRows("SELECT `tp`.`postid`,`tp`.`catid`,`tp`.`subcatid`,`tp`.`title`,`tp`.`image`,
										   `tp`.`url`,`tp`.`description`,`tp`.`totalpostlikes`,`tp`.`cr_date`,`tp`.`description`, 
										   `tc`.`category`,`tsc`.`subcategory`	
										   FROM `tbl_post` AS `tp`
										   INNER JOIN `tbl_category` AS `tc` ON `tp`.`catid` = `tc`.`catid`
										   INNER JOIN `tbl_sub_category` AS `tsc` ON `tp`.`subcatid` = `tsc`.`subcatid`
										   WHERE `tp`.`status`='1' AND `tp`.`catid`='".$catid."' ORDER BY `tp`.`cr_date` DESC LIMIT $OffSet,$PageLimit");
					$alldata = array();
					$responsedata = array();
					foreach($qry21 as $qry21_data)
					{
						$alldata["postid"] = $qry21_data["postid"];
						$alldata["title"] = $qry21_data["title"];
						$alldata["catid"] = $qry21_data["catid"];
					...
					$param = array('success'=>1,'msg'=>'Post found.','totalpage' => $totalpage,'data'=>$responsedata); //Response
				}
				else
				{
			...
			else if(isset($subcatid) && $subcatid!="0")
			{
				/*Pagination Sneaker*/
				$PageLimit = 10;
				$numtotal=qry_numRows("SELECT * FROM `tbl_post` WHERE `status`='1' AND `subcatid`='".$subcatid."'");
				$totalpage=$numtotal/$PageLimit;
				if($numtotal%$PageLimit!=0)
				{
					$totalpage=explode(".",$totalpage);
				...
				if($numtotal > 0)
				{	
					$qry22 = qry_fetchRows("SELECT `tp`.`postid`,`tp`.`catid`,`tp`.`subcatid`,`tp`.`title`,`tp`.`image`,
										   `tp`.`url`,`tp`.`description`,`tp`.`totalpostlikes`,`tp`.`cr_date`,`tp`.`description`, 
										   `tc`.`category`,`tsc`.`subcategory`	
										   FROM `tbl_post` AS `tp`
										   INNER JOIN `tbl_category` AS `tc` ON `tp`.`catid` = `tc`.`catid`
										   INNER JOIN `tbl_sub_category` AS `tsc` ON `tp`.`subcatid` = `tsc`.`subcatid`
										   WHERE `tp`.`status`='1' AND `tp`.`subcatid`='".$subcatid."' ORDER BY `tp`.`cr_date` DESC LIMIT $OffSet,$PageLimit");
					$alldata = array();
					$responsedata = array();
					foreach($qry22 as $qry22_data)
					{
						$alldata["postid"] = $qry22_data["postid"];
						$alldata["title"] = $qry22_data["title"];
						$alldata["catid"] = $qry22_data["catid"];
						$alldata["category"] = $qry22_data["category"];
						$alldata["subcatid"] = $qry22_data["subcatid"];
						$alldata["subcategory"] = $qry22_data["subcategory"];
						$alldata["image"] = $qry22_data["image"];
						$alldata["url"] = $qry22_data["url"];
						$alldata["description"] = $qry22_data["description"];
						$alldata["totalpostlikes"] = $qry22_data["totalpostlikes"];
						$alldata["cr_date"] = $qry22_data["cr_date"];
						$responsedata[] = $alldata;
					}
					$param = array('success'=>1,'msg'=>'Post found.','totalpage' => $totalpage,'data'=>$responsedata); //Response
				}
	...
	case 'GetBookmarkByUserid':
		if(isset($userid))
		{
			...
			foreach($qry0 as $qry0_data)
			{
				...
				if($index !== false)
				{
					$qry_cat = qry_fetchRow("SELECT * FROM `tbl_category` WHERE `catid`='".$qry0_data["catid"]."'");
					$qry_subcat = qry_fetchRow("SELECT * FROM `tbl_sub_category` WHERE `subcatid`='".$qry0_data["subcatid"]."'");
					
					$alldata["postid"] = $qry0_data["postid"];
					$alldata["title"] = $qry0_data["title"];
					$alldata["catid"] = $qry0_data["catid"];
					$alldata["category"] = $qry_cat["category"];
					$alldata["subcatid"] = $qry0_data["subcatid"];
					$alldata["subcategory"] = $qry_subcat["subcategory"];
