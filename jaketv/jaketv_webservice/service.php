...
switch($METHODforCASE) 
{
		...
		if(isset($userid) && $userid=="0")  // New Registration
		{
			...
			else //for Non-social check and using email
			{
				...
				else
				{
					...
					('0','".$fname."','".$lname."','".$email."','".$hashed_pass."','".$pictureurl."','1',NOW(),NOW())"); 
	...
	case 'Login':
		if(isset($email) && isset($password))
		{
				...
				$qry_login = qry_numRows("SELECT * FROM `tbl_user` WHERE `status`='1' AND `email`='".$email."' AND `password`='".$pass."'");
				if($qry_login > 0) 
				{
					$qry_pass = qry_fetchRow("SELECT * FROM `tbl_user` WHERE `status`='1' AND `email`='".$email."' AND `password`='".$pass."'");
	...
	case 'ForgotPassword':
		if(isset($email))
		{
			...
			if($qry15 > 0)
			{
				...
				$mail->SMTPAuth = true;
				$mail->Username = 'jaketvmanager@gmail.com';
				$mail->Password = '1mazeltov';
				/*$mail->Username = 'shivani.hjdimensions@gmail.com';
				$mail->Password = 'Shivani@123#';*/
				$mail->SMTPSecure = 'tls';
				$mail->From = 'noreply@jaketv.com';
	...
	case 'GetPost':
		if(isset($userid) && $userid!="")
		{
			...
			for($i=0; $i<count($cat); $i++){
				$id = $cat[$i];
				$qry021 = qry_fetchRows("SELECT * FROM `tbl_post` WHERE find_in_set($id,catid) AND status='1' AND publish='1' order by postid desc");
				$dd[] = $qry021;
			}
			...
			if($d != ""){
				...
				foreach($qry222 as $qry21_data)
				{
					...
					$cat = explode(',',$qry21_data["catid"]);
                                        ...
                                        $cat2 = array();
					for($i=0;$i<count($cat);$i++){
						$cat1 = qry_fetchRow("select * from tbl_category where catidu='".$cat[$i]."'");
						//$cat2[] = $cat1;
...
//					}	
		...
		if(isset($userid))
		{
			if(isset($catid) && $catid!="0")
			{
				...
				$category_name = qry_fetchRow("SELECT category FROM `tbl_category` WHERE `catidu`=$catid");
					...
					$maincat = "SELECT * FROM `tbl_post` WHERE `status`='1' AND publish='1' AND FIND_IN_SET($catid,catid)";
	...
	case 'GetBookmarkByUserid':
		if(isset($userid))
		{
			$alldata = array();
			$responsedata = array();
			$qry0 = qry_fetchRows("SELECT * FROM `tbl_post` WHERE `status`='1' AND publish='1' ORDER BY `cr_date` DESC");
			foreach($qry0 as $qry0_data)
			{
	...
	case 'ChangePassword':
		if(isset($userid) && $userid!='0' && $userid!='')
		{
			...
			{
				$pass = hash('sha512', $password);
				$changepassword = "UPDATE `tbl_user` SET password='".$pass."' WHERE `userid`='".$userid."'";
