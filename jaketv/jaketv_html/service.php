...
switch($METHODforCASE) 
{
	...
	case 'GetPost':
		if(isset($userid) && $userid!="")
		{
			...
			/*Pagination Sneaker*/
			$PageLimit =25;
			$numtotal=qry_numRows("SELECT DISTINCT `tp`.*,`tc`.`catidu`,`tc`.`category`,`tsc`.`subcatidu`,`tsc`.`subcategory` FROM `tbl_category` AS `tc` 
								    INNER JOIN `tbl_post` AS `tp` ON `tc`.`catidu`=`tp`.`catid` 
								    INNER JOIN `tbl_sub_category` AS `tsc` ON `tp`.`subcatid`=`tsc`.`subcatidu`
								    WHERE `tp`.`status`=1 AND (`tp`.`catid` IN (".$catids.") OR `tp`.`subcatid` IN (".$catids.")) ORDER BY `tp`.`postid` DESC");
			...
			$qry222 = qry_fetchRows("SELECT `tp`.*,`tc`.`catidu`,`tc`.`category`,`tsc`.`subcatidu`,`tsc`.`subcategory` FROM `tbl_category` AS `tc` 
								    INNER JOIN `tbl_post` AS `tp` ON `tc`.`catidu`=`tp`.`catid` 
								    INNER JOIN `tbl_sub_category` AS `tsc` ON `tp`.`subcatid`=`tsc`.`subcatidu`
								    WHERE `tp`.`status`=1 AND (`tp`.`catid` IN (".$catids.") OR `tp`.`subcatid` IN (".$catids.")) ORDER BY `tp`.`postid` DESC LIMIT $OffSet,$PageLimit");
